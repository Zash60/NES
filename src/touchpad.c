...
void init_touch_pad(struct Emulator* emulator){
    ...
    TTF_Font* small_font = TTF_OpenFont("asap.ttf", (int)(font_size * 0.8));
    ...
    int top_y = (int)(ctx->screen_height * 0.08); 
    int center_x = ctx->screen_width / 2;
    int tas_y = top_y + (int)(ctx->screen_height * 0.12); 
    ...
    // TAS
    init_button(&touch_pad.tas_toggle, BTN_ID_TAS_TOGGLE, 9, BUTTON_SMALL, "TAS", center_x + (int)(ctx->screen_width * 0.15), top_y, small_font);
    
    int tas_spacing = (int)(ctx->screen_width * 0.09);
    int tas_start_x = center_x - (tas_spacing * 2);
...
static void init_button(struct TouchButton* button, uint32_t id, size_t index, int type, char* label, int x, int y, TTF_Font* font){
    ...
    if(type == BUTTON_SMALL) { w = text_surf->w + 20; h = text_surf->h + 10; }
    int r = h / 2;
...
void render_touch_controls(GraphicsContext* ctx){
    ...
    for(int i = 0; i < TOUCH_BUTTON_COUNT; i++){
        ...
        if (button->id == BTN_ID_TAS_REC) { if (touch_pad.emulator->is_recording) SDL_SetTextureColorMod(button->texture, 255, 0, 0); else SDL_SetTextureColorMod(button->texture, 255, 255, 255); } 
        else if (button->id == BTN_ID_TAS_PLAY) { if (touch_pad.emulator->is_playing) SDL_SetTextureColorMod(button->texture, 0, 255, 0); else SDL_SetTextureColorMod(button->texture, 255, 255, 255); }
        ...
        // Botão LUA (antigo Box) não muda de cor, apenas carrega
        ...
        case SDL_EVENT_FINGER_DOWN: {
            ...
            for (int i = 0; i < TOUCH_BUTTON_COUNT; i++) {
                ...
                if ((i < 4 && is_within_circle((int)x, (int)y, btn->x, btn->y, btn->r)) || (i >= 4 && is_within_bound((int)x, (int)y, &btn->dest))) {
                    ...
                    else if (btn->id == BTN_ID_TAS_REC) tas_toggle_recording(touch_pad.emulator);
                    else if (btn->id == BTN_ID_TAS_PLAY) tas_toggle_playback(touch_pad.emulator);
                    ...
                    else if (btn->id == BTN_ID_TAS_BOX) tas_toggle_lua_script(touch_pad.emulator); // Carrega Script
...
void free_touch_pad(){ if(!touch_pad.g_ctx) return; for(int i = 0; i < TOUCH_BUTTON_COUNT; i++) if(touch_pad.buttons[i] && touch_pad.buttons[i]->texture) SDL_DestroyTexture(touch_pad.buttons[i]->texture); SDL_DestroyTexture(touch_pad.axis.joy_tx); SDL_DestroyTexture(touch_pad.axis.bg_tx); }
#endif
