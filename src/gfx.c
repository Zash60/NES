...
void get_graphics_context(GraphicsContext* ctx){
    ...
    ctx->dest.h = (float)want_h; ctx->dest.w = (float)want_w; ctx->dest.x = (float)(ctx->screen_width - want_w) / 2; ctx->dest.y = 0;
#else
...
// 1. Atualiza a textura com os pixels do emulador (apenas desenha o jogo)
...
// 2. Desenha UI e finaliza o frame
...
#ifdef __ANDROID__
    start_x = g_ctx->dest.x; width = g_ctx->dest.w; start_y = g_ctx->dest.y; end_y = start_y + g_ctx->dest.h;
#else
    start_x = 0; width = g_ctx->width * g_ctx->scale; start_y = 0; end_y = g_ctx->height * g_ctx->scale;
#endif
    int step = (end_y - start_y > 1000) ? 3 : 2;
    for (float y = start_y; y < end_y; y += step) SDL_RenderLine(g_ctx->renderer, start_x, y, start_x + width, y);
    SDL_SetRenderDrawBlendMode(g_ctx->renderer, SDL_BLENDMODE_NONE);
}

static void render_fps_text(GraphicsContext* ctx, float fps) {
    if (!ctx->font) return;
    char fps_str[16]; snprintf(fps_str, sizeof(fps_str), "%.0f", fps);
...
void free_graphics(GraphicsContext* ctx){
    ...
    TTF_Quit(); SDL_DestroyTexture(ctx->texture); SDL_DestroyRenderer(ctx->renderer); SDL_DestroyWindow(ctx->window); SDL_DestroyAudioStream(ctx->audio_stream); SDL_Quit();
}
